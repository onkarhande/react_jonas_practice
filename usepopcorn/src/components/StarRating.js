...
export default function StarRating({ maxRating = 5, color = '#fcc419', size = 48, onSetRating, defaultRating = 0 }) {
    ...
    return (
        <div className='container'>
            <div className='starContainer'>{Array.from({ length: maxRating }, (_, i) => (
...
function Star({ onRate, isFull, hoverIn, hoverOut, color, size }) {
    const StarStyle = {
        width: `${size}px`,
        height: `${size}px`,
        display: "block"
    ...
    return (
        <span style={StarStyle} onClick={onRate} onMouseEnter={hoverIn} onMouseLeave={hoverOut}>
            {isFull ? (
                <svg
