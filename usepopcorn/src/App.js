...
export default function App() {
  ...
  function handleDeleteWatched(id) {
    setWatched(watched => watched.filter(o => o.imdbID !== id));
  }
  ...
  useEffect(function () {
    ...
    async function fetchData() {
      try {
        let data = await fetch(`http://www.omdbapi.com/?apikey=43d6dbab&s=${query}`, { signal: controller.signal });
        if (!data.ok) {
        ...
        <Box>
          {selectedId ?
            <MovieDetails selectedId={selectedId} onCloseMovie={handleCloseMovie}
              onAddWatched={handleAddWatched} watched={watched} onDeleteWatched={handleDeleteWatched}
            ></MovieDetails>
            :
            <></>}
